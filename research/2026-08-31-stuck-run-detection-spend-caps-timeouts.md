...
- **Semantics under subscription auth are unverified.** The Loop bills to a Max subscription (claude.sh actively refuses metered keys). The costs page says for subscribers the session cost figure "isn't relevant for billing purposes"; whether `--max-budget-usd` still trips on the estimated figure under subscription login is not stated anywhere read for this note. **UNVERIFIED — test with a 1-cent cap before relying on it.**
...
1. **Whether `--max-budget-usd` trips under subscription (OAuth) authentication.** The docs describe it as a cap on "API calls" and separately say subscriber session-cost figures aren't billing-relevant; no source read states the flag's behavior on a Max login. Needs a live 1-cent test on the Loop box. UNVERIFIED.
