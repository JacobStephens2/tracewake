...
- `docs/adr/` - the decisions, numbered. ADRs 0003-0013 govern the Loop.
...
- `wizards/` - runnable walkthroughs for the steps only a human can take.
  `loop-sbx-login.sh` signs the Loop's Execution Boundary in, and
  `loop-github-credentials.sh` registers the Loop's signing key and installs its
  repository-scoped token - all three steps need a browser the box does not
  have.
