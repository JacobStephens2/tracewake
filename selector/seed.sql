...
-- strip's budget cell reads `7 of 4` against this fixture. That is arithmetic
-- rather than breakage. The cap is 4 dispatches per rolling 24 hours and the
-- Selector refuses the fifth, so seven inside one day is unreachable in life -
-- but this file packs every outcome state into the last hour on purpose, and
...
-- 5. The box, as the status strip's box card renders it (#156). A cycle reads
